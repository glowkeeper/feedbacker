...
| Check | Result |
| --- | --- |
| Parity with Python, 6 cases (`npm run parity`) | All pass. The parsed results are identical. Every page's image/text classification, character count, text lines and line colours are identical too (102–103 lines per marked view), with two explained differences (below). |
| The replica's key facts, plus operator-level checks (`npm test`, 13 tests) | ID, word count, grade `60 /100`, general comment, comments 1–3 with tags, pages and positions `0.301, 0.621, 0.451`, rubric total `59.75`, and four criteria with IMPLEMENTATION's selected `2:2 (55)` against its awarded `58`. No warnings. |
...
4. **`cmyk`**: the replica with every colour given as CMYK, as print-oriented producers write it. Both parsers select the same levels (see "CMYK colours" below).
5. **`not-a-pdf`**: both parsers fail clearly. Only the library's exception name differs.
6. **`chrome-printed`**: the same layout written as HTML and printed to PDF by Chrome (`scripts/make_browser_case.ts`). Browser-printed PDFs use scaled transforms, positioned glyph runs and embedded subset fonts. That's closer to how real current views are likely produced, and much harder than reportlab's output. It matches exactly.

All cases are fictional. No real material was used.

### The explained differences

- **Undecodable glyphs.** pdfminer can't map the replica's bullet glyph to Unicode and writes `(cid:127)`; pdf.js decodes it as `•`. The parser never reads those description lines, so the results are unaffected. It's a difference in pdf.js's favour. The parity check accepts only this pattern and reports each occurrence.
- **CMYK darkness values.** pdf.js converts CMYK to RGB with its own display curve, and Python with the plain formula. Line darkness therefore differs in value but not in order, so the same levels are selected. The parity check accepts this only for the `cmyk` case, and still requires identical parsed results.

### CMYK colours (decided 2026-09-26)

The maintainer decided that **the selected level is found by true darkness, whatever the colour space.**

The spike exposed a bug in the Python reference. Its `_darkness` read the C, M and Y values as if they were R, G and B and ignored K, so every grey CMYK line looked equally dark. A CMYK marked view therefore gave "could not be identified" for every criterion. That was safe, but for the wrong reason. It also couldn't be matched in TypeScript, because pdf.js converts CMYK before the parser sees it.

`core/src/feedbacker_core/marked_view.py` now converts CMYK (and grey) to luminance, with core tests for RGB, grey and CMYK, and for a CMYK near-tie that must still warn. The selection rule is unchanged: the darkest level must stand out by at least 0.1, or the parser warns rather than guesses. Imported marks still require the moderator's confirmation.
...
- **Version-specific operator shapes.** pdf.js 6 passes the `Tm` operands as one array, and older versions passed them flat; both are accepted. Pinning pdf.js and keeping the operator-level tests (`test/page.test.ts`) guards against further changes.
- **Covered beyond the parity cases:** text placed with `Tm`, `TD`, `TL`, `T*` and `'`; scaling by the text matrix and the current transform; fonts set through an ExtGState; glyph widths in fonts with their own matrix (Type3); and every pdf.js image operator, including its merged "repeat" and "group" forms.
...
npm run typecheck
npm run parity       # Python vs TypeScript on the 6 cases (needs uv and the core env, and Chrome for case 6)
